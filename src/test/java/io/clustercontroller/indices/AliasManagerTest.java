...
public class AliasManagerTest {
    ...
    @Test
    void testCreateAlias_UpdatesIndexConfig() throws Exception {
        // Arrange
        String aliasName = "test-alias";
        String indexName = "test-monday";
        String aliasConfig = "{}";
        String indexConfigJson = "{\"id\":\"test-monday\",\"aliases\":{}}";
        
        // Mock empty goal state initially
        when(metadataStore.getCoordinatorGoalState(testClusterId)).thenReturn(null);
        when(metadataStore.getIndexConfig(testClusterId, indexName)).thenReturn(Optional.of(indexConfigJson));
        doNothing().when(metadataStore).setAlias(eq(testClusterId), eq(aliasName), any(Alias.class));
        doNothing().when(metadataStore).setCoordinatorGoalState(eq(testClusterId), any(CoordinatorGoalState.class));
        doNothing().when(metadataStore).updateIndexConfig(eq(testClusterId), eq(indexName), anyString());
        
        // Act
        aliasManager.createAlias(testClusterId, aliasName, indexName, aliasConfig);
        
        // Assert - verify index config was retrieved twice (once for validation, once for update)
        verify(metadataStore, times(2)).getIndexConfig(testClusterId, indexName);
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq(indexName), argThat(config -> 
            config.contains("\"test-alias\"")
        ));
    }
    
    @Test
    void testDeleteAlias_UpdatesIndexConfig() throws Exception {
        // Arrange
        String aliasName = "test-alias";
        String indexName = "test-monday";
        String indexConfigJson = "{\"id\":\"test-monday\",\"aliases\":{\"test-alias\":{}}}";
        
        CoordinatorGoalState goalState = new CoordinatorGoalState();
        goalState.getRemoteShards().getAliases().put(aliasName, indexName);
        
        when(metadataStore.getCoordinatorGoalState(testClusterId)).thenReturn(goalState);
        when(metadataStore.getIndexConfig(testClusterId, indexName)).thenReturn(Optional.of(indexConfigJson));
        doNothing().when(metadataStore).deleteAlias(testClusterId, aliasName);
        doNothing().when(metadataStore).setCoordinatorGoalState(eq(testClusterId), any(CoordinatorGoalState.class));
        doNothing().when(metadataStore).updateIndexConfig(eq(testClusterId), eq(indexName), anyString());
        ...
        // Assert - verify index config was retrieved and updated
        verify(metadataStore).getIndexConfig(testClusterId, indexName);
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq(indexName), argThat(config -> 
            !config.contains("\"test-alias\"")
        ));
    }
    ...
    @Test
    void testCreateAlias_MultiIndex_UpdatesAllIndexConfigs() throws Exception {
        // Arrange
        String aliasName = "test-multi-alias";
        String indexNames = "test-monday,test-tuesday";
        String aliasConfig = "{}";
        String mondayConfigJson = "{\"id\":\"test-monday\",\"aliases\":{}}";
        String tuesdayConfigJson = "{\"id\":\"test-tuesday\",\"aliases\":{}}";
        
        // Mock empty goal state initially
        when(metadataStore.getCoordinatorGoalState(testClusterId)).thenReturn(null);
        when(metadataStore.getIndexConfig(testClusterId, "test-monday")).thenReturn(Optional.of(mondayConfigJson));
        when(metadataStore.getIndexConfig(testClusterId, "test-tuesday")).thenReturn(Optional.of(tuesdayConfigJson));
        doNothing().when(metadataStore).setAlias(eq(testClusterId), eq(aliasName), any(Alias.class));
        doNothing().when(metadataStore).setCoordinatorGoalState(eq(testClusterId), any(CoordinatorGoalState.class));
        doNothing().when(metadataStore).updateIndexConfig(anyString(), anyString(), anyString());
        
        // Act
        aliasManager.createAlias(testClusterId, aliasName, indexNames, aliasConfig);
        
        // Assert - verify both index configs were retrieved twice each (once for validation, once for update)
        verify(metadataStore, times(2)).getIndexConfig(testClusterId, "test-monday");
        verify(metadataStore, times(2)).getIndexConfig(testClusterId, "test-tuesday");
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq("test-monday"), argThat(config -> 
            config.contains("\"test-multi-alias\"")
        ));
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq("test-tuesday"), argThat(config -> 
            config.contains("\"test-multi-alias\"")
        ));
    }
    
    @Test
    void testDeleteEntireAlias_UpdatesAllIndexConfigs() throws Exception {
        // Arrange
        String aliasName = "test-multi-alias";
        List<String> targetIndices = Arrays.asList("test-monday", "test-tuesday");
        String mondayConfigJson = "{\"id\":\"test-monday\",\"aliases\":{\"test-multi-alias\":{}}}";
        String tuesdayConfigJson = "{\"id\":\"test-tuesday\",\"aliases\":{\"test-multi-alias\":{}}}";
        ...
        // Act - delete entire alias (pass null/empty indexName)
        aliasManager.deleteAlias(testClusterId, aliasName, null);
        
        // Assert - verify both index configs were retrieved and updated
        verify(metadataStore).getIndexConfig(testClusterId, "test-monday");
        verify(metadataStore).getIndexConfig(testClusterId, "test-tuesday");
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq("test-monday"), argThat(config -> 
            !config.contains("\"test-multi-alias\"")
        ));
        verify(metadataStore).updateIndexConfig(eq(testClusterId), eq("test-tuesday"), argThat(config -> 
